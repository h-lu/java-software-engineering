...
- `src/test/java/com/campusflow/quality/TaskStatusCalculatorCoverageTodoTest.java`：禁用的覆盖率模板，完成分支测试后移除 `@Disabled`。
...
内置测试只覆盖最小 smoke path。`TaskStatusCalculatorCoverageTodoTest` 负责提示你补齐 completed、unscheduled、overdue 分支，再结合 JaCoCo 校验覆盖率。
