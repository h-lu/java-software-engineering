...
> "质量是免费的，但它不是礼物。你必须首先理解它，然后付出努力去实现它。"
> —— W. Edwards Deming（质量管理之父，对软件工程产生深远影响）
...
- 本周改进：引入 SpotBugs/PMD 静态分析，配置 JaCoCo 覆盖率，建立质量门禁
...
## 2. 让机器帮你"扫雷"——SpotBugs 入门

老潘打开 Maven 配置文件。

"先从 SpotBugs 开始。"老潘说，"它是 Java 领域最成熟的静态分析工具之一，能发现空指针、资源泄漏、性能问题等。"
...
"别慌，"老潘说，"先看优先级。SpotBugs 把问题分类为高、中、低。先修复高优先级。"
...
"如果 `findById` 返回 `null`，下一行直接调用方法就会抛 `NullPointerException`。"老潘解释，"这就是 SpotBugs 发现的问题——你没有检查 `null`。"
...
"现在 SpotBugs 只会扫描主代码，并且只报告高优先级问题。"老潘说，"运行一下试试。"
...
SpotBugs 已经能自动发现问题了，那为什么还需要人工审查？阿码忍不住插话："那 AI 不能直接帮我修复吗？"
...
<!--
**Bloom 层次**：应用
**学习目标**：能够配置 SpotBugs，理解并修复常见警告（空指针、资源泄漏等）
**贯穿案例推进**：使用 SpotBugs 扫描 CampusFlow 代码，发现并修复至少 3 个问题
**建议示例文件**：02_spotbugs_config.xml, 02_spotbugs_report.html, 02_nullpointer_fix.java
**叙事入口**：从"运行 SpotBugs 发现 12 个问题"的真实场景切入
**角色出场**：
- 小北：看到 SpotBugs 报告的惊讶，学习如何修复空指针问题
- 阿码：质疑为什么不直接用 AI 修复
- 老潘：讲解 SpotBugs 的使用，强调工具与人分工
**回顾桥**：
...
"SpotBugs 找出了潜在的 bug，但你怎么知道测试覆盖了足够多的代码？"老潘问。
...
"这个配置的意思是：SpotBugs 的高优先级问题必须修复才能合并，覆盖率低于 70% 会警告但允许合并。"
...
**贯穿案例推进**：为 CampusFlow 的 SpotBugs 警告建立技术债 Backlog，排定修复优先级
...
- SpotBugs 发现 12 个问题，修复了 4 个高优先级的
- 覆盖率从 68% 提升到 75%
...
```
feat(quality): add SpotBugs and JaCoCo configuration
...
SpotBugs 让你看到了代码的"隐形冰山"——空指针、资源泄漏、未使用的字段。这些问题可能不会立即导致崩溃，但它们是"定时炸弹"，会在最糟糕的时候爆炸。通过静态分析，你能提前发现并拆除这些炸弹。
...
- [ ] 我能配置 SpotBugs，理解并修复常见警告（空指针、资源泄漏等）
- [ ] 我能使用 JaCoCo 测量代码覆盖率，分析测试盲区
- [ ] 我能为项目设计合理的质量门禁策略，平衡质量与速度
- [ ] 我能为技术债排定优先级，制定合理的偿还计划
- [ ] 我完成了 CampusFlow 的 SpotBugs 配置，修复了至少 3 个高优先级问题
