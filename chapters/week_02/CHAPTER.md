...
（数据来源：Ox Security 2025 年研究报告、Forrester 2026 技术预测，详见 AI 小专栏参考链接）
...
小北还是有点困惑："基因这个比喻我还是不太明白……"

老潘换了种说法："那换个更直观的比喻——**类就像户型图，对象就像按照户型图盖出来的实际房子**。"

- 户型图（类）规定了这个户型有 3 个卧室、2 个卫生间
- 但每套房子（对象）里住的人不同、装修风格不同、家具摆设不同
- 你可以用同一张户型图盖 100 套房子——每套房子有自己独立的状态

"所以 `Task` 类就像户型图，规定每个任务有 title、completed 这些'房间'，"老潘说，"而 `task1`、`task2` 是按照这个模板创建出来的'实际房子'——每个任务有自己的 title 值，就像每套房子有自己的住户。"
...
public class Task {
    ...
    public void setPriority(String priority) {
        // setter 可以加验证逻辑，确保优先级在合法范围内
        if (!priority.equals("高") && !priority.equals("中") && !priority.equals("低")) {
            throw new IllegalArgumentException("优先级必须是'高'、'中'或'低'");
        }
...
"这里用到的 `interface` 是 Week 08 才会深入学习的内容。现在只需要理解'通过某种方式扩展功能，而不是修改现有代码'这个思想。"老潘补充，"接口定义了'能做什么'，具体类实现'怎么做'——这个概念你先有个印象就行，不用急着深究。"
...
>
> 2025 年的研究报告显示了一个矛盾：开发者用 AI 写代码更快了（平均节省 55% 时间），但代码质量反而下降了——研究表明相当比例的 AI 生成代码存在"职责不清晰"的问题。
>
> 更关键的是，多项研究显示 **AI 生成的代码中，代码重复量显著增加**。Syncfusion 2025 年的报告指出："AI 生成的代码往往缺乏结构，导致系统脆弱"。这些"能跑但设计混乱"的代码会快速累积成技术债：当需求变化时（而需求总是在变化），修改 AI 生成的代码比修改手工设计的代码**更困难**——因为你不理解当初 AI 为什么这样生成，也不知道改一个地方会不会影响其他地方。
>
...
### 方案 B：使用 Map 存储所有任务属性
- **优点**：灵活性高，可以动态添加属性
- **缺点**：类型不安全，容易出错
- **结论**：不采纳 —— 牺牲了 Java 静态类型的优势
...
### 设计约束
- 所有 Task 字段都是 `private`（封装）
- title、description 等字段是 `final`（创建后不可变）
- 只有 completed 状态可以修改（通过 markCompleted()）
...
1. **名词提取法**：类不是凭空想出来的，而是从问题域中识别出来的。实体类存储数据，服务类提供行为，两者职责要分清楚。

2. **封装的三层价值**：`private` 字段 + getter/setter 不是"模板代码"，而是把"外部如何访问"和"内部如何存储"解耦开来。内部实现可以自由变化（如 `boolean` 改 `enum`），只要公共接口不变，外部代码完全不受影响。

3. **单一职责原则（SRP）**：一个类应该只有一个引起它变化的原因。上帝类看起来方便，但每次需求变都要改它，维护成本极高。拆分后，改验证规则只需动 `TaskValidator`，不会影响 `Task`。

4. **ADR 记录"为什么"**：代码告诉你"怎么做"，ADR 告诉你"为什么做"。ADR 必须由人来写——它记录的是团队的决策过程，不是通用答案。

5. **AI 是工具，你是设计者**：AI 能生成类代码，但不能做设计决策。SOLID 原则、名词提取法、封装思想——这些是"审查 AI 代码、判断设计质量"的标尺。

### 本周关键词

| 概念 | 一句话记住 |
|------|----------|
| 上帝类 | 职责过多的巨型类，改一处可能影响全局 |
| 封装 | private 字段 + public 方法，保护数据，控制变化 |
| SRP | 一个类只有一个引起变化的原因 |
| ADR | 记录"为什么这样设计"的轻量文档 |
...
| 类名 | 类型 | 职责 | 设计要点 |
|------|------|------|---------|
| Task | 实体类 | 存储任务数据 | private 字段 + getter/setter，setter 含验证逻辑 |
| TaskManager | 服务类 | 管理任务增删改查 | 单一职责，依赖 TaskValidator |
| TaskValidator | 服务类 | 验证数据合法性 | 无状态，可复用 |

完整代码实现见 `examples/05_campusflow_domain.java`，与正文第 2 节示例设计一致。

### ADR-001：领域模型设计

本周你撰写了 ADR-001，记录核心设计决策：
- Task 和 TaskManager 分离的原因（单一职责）
- 优先级用 String 而非枚举的权衡（Week 08 会重构）
- TaskValidator 独立出来的好处（验证逻辑复用）

### 未来展望

| 周次 | 计划 |
|------|------|
| Week 03 | 添加异常处理，输入不合法时优雅反馈 |
| Week 04 | 构建 CLI 交互界面，实现命令行操作 |
| Week 07 | 添加数据持久化，任务保存到数据库 |
