...
**设计建议**：
- 所有自定义异常应继承 `CampusFlowException`（受检异常）
- `InvalidTaskDataException` 与 `DuplicateTaskException` 的区别：
  - `InvalidTaskDataException`：数据格式/值无效（如空标题、非法日期）
  - `DuplicateTaskException`：业务规则违反（任务已存在）
- 两者可以并列声明，因为代表不同的失败场景
