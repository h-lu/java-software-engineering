...
2. 如果是 Python 3，`age = "27"` 然后 `age + 10` 会发生什么？那 `age + "10"` 呢？哪种情况更危险？
...
- **数字验证**：直接复制下面的代码示例即可，里面的 `"\\d+"` 是一个"魔法咒语"，你只需要知道它能判断"输入是不是全是数字"就行
...
**第一步（基础）**：找出 2-3 个明显的问题
- [ ] 代码能正常运行吗？如果运行看看会发生什么
- [ ] 如果输入"张三 李四"（带空格的名字），会输出什么？

**第二步（进阶）**：深入分析
- [ ] `scanner.next()` 和 `scanner.nextLine()` 有什么区别？
- [ ] `scanner.nextInt()` 之后会发生什么？
- [ ] 有没有资源没有正确关闭？
