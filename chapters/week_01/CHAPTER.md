# Week 01：从 Python 到 Java——抽象层次与软件工程史
...
老潘在第一节课上就问："有了 AI，我们为什么还要学软件工程史？"

底下一片沉默。阿码小声嘀咕："历史课总是很无聊……"
...
> **第一次读可以跳到这里**
>
> 如果你对历史不太感兴趣，只想快速上手 Java，可以直接跳到[从 Hello World 开始](#从-hello-world-开始)写第一个程序。但建议稍后回来——理解"为什么 Java 要这样设计"会让你少踩很多坑。

### 从机器指令到对象

**第一个时代：机器语言时代**。程序员直接和硬件对话，用 0 和 1 编写指令。每行代码都在告诉 CPU"把这个值移到那个寄存器"。

老潘在白板上写了一段"伪机器码"：

```
10110000 00000001    ; 把数字 1 放进寄存器 A
10110001 00001010    ; 把数字 10 放进寄存器 B
00000100 00000001    ; A 和 B 相加，结果存回 A
```

"这是最低层次的抽象。你写一个'打印名字'的功能，需要几十行这样的指令，而且稍微改一点需求——比如从打印'张三'改成打印'李四'——你就要重新计算内存地址、重写大半代码。"

老潘问："如果让你用机器语言写一个电商网站，你愿意吗？"

"疯了。"小北摇头，"写个首页都要写半年。"

"所以出现了**第二个时代：高级语言时代**。"老潘擦掉了伪代码，写下：

```c
printf("Hello, World!");
```

"C 语言让你写 `printf`，编译器帮你翻译成刚才那堆 0 和 1。抽象层次提升了——你不用关心寄存器、内存地址，你关心的是'我要做什么'。"

"但这还不够。"老潘继续画，"程序越来越大，几个人的项目变成几十人的团队，代码从几千行变成几十万行。**函数式的抽象不够用了**——你需要一种更贴近现实世界的思维方式。"
...
"把'学生'、'课程'、'选课记录'当成一个个'对象'，每个对象有自己的属性和行为。这就是**第三个时代：面向对象时代**。Java 就是在这个背景下诞生的（1995年）。它不是第一个面向对象语言，但它把'封装、继承、多态'这套思维方式带进了主流工程。"
...
他顿了顿，"上周我用 AI 生成了一段读取用户输入的代码，看起来没问题。但测试时发现——如果用户直接按回车不输入任何内容，程序就崩了。AI 不会告诉你这种边界情况，它只会生成'理想情况'下的代码。"

### 从 Hello World 开始

理解了抽象层次的演进，我们来写第一个 Java 程序——一个最简单的"个人名片生成器"。先从最低的抽象层次开始：硬编码输出。
...
这段代码是最原始的版本：所有信息都写死在代码里。每次想生成不同的名片，你都要修改源代码、重新编译、运行。

在机器语言时代，这已经是很"高级"的抽象了。但在今天，我们知道它不够灵活。
...
在 Python 里，变量像便利贴——你随时可以把它贴到任何值上。解释器会在运行时"猜"你想做什么：

```python
name = "张三"  # Python 看到"这是个字符串"
name = 123     # 没问题，现在 name 是整数了
```

Python 只是默默把便利贴贴到了新的值上。
...
老潘顿了顿："但在 Java 里，你同事写 `String discount = "8折";` 的瞬间，IDE 就会标红。代码根本跑不起来。"

这就是静态类型的价值：**把错误从运行时提前到编译期**。

"你宁可多写几个字符的类型声明，"老潘说，"也不愿意半夜三点被电话叫起来修复线上 bug。"
...
阿码突然举手："那 `String` 不是基本类型，为什么可以直接写 `String name = "张三"`，不用像 `int` 那样用 `new` 创建？"
...
理解类型系统给了你一个审查 AI 代码的标尺。现在让我们把这个能力用起来——写一个能跟用户"对话"的程序。

## 3. 程序是活的——跟用户聊起来

小北把名片生成器发给同学试用，五分钟后收到微信："我想生成李四的名片，怎么改？"

他盯着代码叹了口气——这意味着要改源代码、重新编译、再发一遍 jar 包。

"要是能让用户自己输入就好了……"小北嘟囔着。

阿码刚好路过，探头看了一眼："能不能像 Python 的 `input()` 那样，运行时问用户要信息？"

"Java 也有这功能，"老潘不知什么时候站在了他们身后，"叫 `Scanner`，标准库里的。"
...
> **提前说明**：这一节讲的是 `nextInt()` 和 `nextLine()` 混用时的问题。如果你觉得理解起来有困难，可以先记住这个**简单规则**：**读取字符串用 `nextLine()`，读取数字也用 `Integer.parseInt(scanner.nextLine())`**，就不会遇到下面这个问题了。学完这一节后，你会理解为什么要这样做。
...
"轮到 `nextLine()` 时，它看到杯底还有个换行符，就以为'哦，这一行已经结束了'，直接返回空字符串。"
...
这种"严格"有时候让人烦躁，但它是有目的的。让我们通过一个具体的对比，看看 Python 的"宽容"和 Java 的"严格"在实际中会产生什么不同的结果。

---

## 4. "这太严格了吧"——从 Python 的宽容到 Java 的严谨

写完 Scanner 程序后，小北开始有点怀念 Python 了。
...
# 场景 1：想当然地做加法
result = age + 10  # ❌ TypeError: 字符串不能直接加整数
# Python 3 会直接报错，不会静默失败

# 场景 2：如果不小心写成字符串拼接
age_str = age + "10"  # 静默得到 "2510"，bug 很隐蔽！
print(f"十年后 {name} 的年龄是 {age_str}")
```

"你看，"老潘指着代码，"`input()` 返回的是字符串。Python 3 中，`age + 10` 会直接抛出 TypeError——这是好消息。但如果你写成 `age + '10'`，Python 会静默地拼接成 `'2510'`，这个 bug 很隐蔽，可能到测试甚至上线才发现。"
...
阿码马上追问："那如果我先用 Python 做个原型，后面要转成 Java，会不会很麻烦？"

"这是个好问题。"老潘说，"小项目用 Python 快速验证想法，验证成功后再用 Java 重构，这是很常见的做法。重构时类型系统会帮你发现很多隐藏的问题。"

### 类型安全的好处

阿码举手了："但我还是觉得 Java 太严格了。能不能举个例子，说明'严格'真的有用？"

老潘想了想，讲了他几个月前Review过的一段代码：

"那是个库存管理系统，Python写的。核心逻辑看起来很简单："
...
"问题出在调用的地方。有人重构时不小心把参数顺序写反了："

```python
update_stock(100, "ITEM-001")  # 应该是 ("ITEM-001", 100)
```

"Python不会报错——它把字符串`"ITEM-001"`当成数字100来减，结果库存变成了负数。更可怕的是，这个bug在测试环境没被发现，因为测试用的item_id恰好也是数字。"
...
"你传反了参数？编译器直接报错：`int cannot be converted to String`。代码根本跑不起来，bug被扼杀在摇篮里。"

老潘总结道："类型系统就像一道安检门。Python是'先上车后补票'，出问题了你已经在高速上；Java是'先安检再进站'，危险品根本进不来。"
...
到这一步，你的名片生成器已经能跑起来了。但老潘看了一眼代码，说了一句话："能跑和能维护是两回事。"

在把代码交给 AI 帮你"优化"之前，你得先知道什么是真正的好代码——否则 AI 给你的可能只是"另一种能跑的写法"。
...
>
> 阿码试过："我让 AI 生成一个用户登录模块，它代码写得很快，但完全没考虑'用户输错 3 次密码就要锁定'这种业务规则。"
>
...
老潘说得对——知道坑在哪里，只是第一步。真正的工程师，还要写出别人（包括三个月后的自己）能看懂的代码。

## 5. 工程不是写完代码——从"能跑"到"可维护"

名片生成器现在能跑了。小北得意地把代码发给老潘看。

老潘扫了一眼，摇摇头："能跑，但三个月后你自己都看不懂。"

### 代码可读性的重要性

小北不服："怎么会看不懂？我自己写的啊。"

"做个实验。"老潘说，"打开你三个月前写的 Python 脚本，不看注释，告诉我这段代码在做什么。"

小北打开了一个文件，盯着看了五分钟。

"……好像是个爬虫？"他尴尬地挠头，"但这个变量 `x` 是干嘛的来着……"

"这就是问题。"老潘说，"**代码写一次，会被读十次。** 包括你未来的自己、你的同事、接手你项目的人。"

小北突然理解了为什么古人说"书到用时方恨少"——现在他想说的是"代码到改时才恨乱"。
...
"**注释应该解释'为什么'，不是'是什么'。** 代码本身已经告诉你它在做什么了，不需要翻译成中文。"
...
好注释解释**为什么这么做**——避免换行符问题、匹配中文字符宽度——而不是翻译代码。
...
重构后的代码看起来比原来长，但结构更清晰。

> **⚠️ 代码阅读提示**
>
> 下面的代码有些语法你可能没见过（`private`、`final`、`getter` 之类）。**不用担心！**
> - **本周目标**：理解"代码被拆成了几个方法"这个结构
> - **下周会讲**：`private`、`final` 等关键字的含义
> - **现在可以跳过**：如果你觉得代码太长，可以先看 `main` 方法，理解整体流程即可
>
> 重点看这三点：
> 1. `main` 方法变短了——只负责"调用"
> 2. 出现了 `readUserInfo()`、`printBusinessCard()` 等方法——各司其职
> 3. 用户信息被封装成 `UserInfo` 对象——不用传 5 个参数
...
小北点点头，似懂非懂。老潘拍了拍他的肩膀："不急，你现在只需要记住一个原则：**代码不只是写给计算机看的，更是写给未来的自己和队友看的。**"

---

稍微喘口气。刚才一下子介绍了 `private`、`final`、`getter` 这些概念，你可能觉得信息量有点大。没关系——本周的目标是"知道这些概念存在"，Week 02 和 Week 03 会深入讲解。现在我们换个角度，聊聊这些工程思维和 AI 时代的关系。
...
"跑起来了，"阿码说，"但后来我加了个需求——让学生能选多个课程——发现根本改不动。所有代码都堆在一个文件里，变量名叫 `a`、`b`、`temp`……"

"上周我们团队就踩了这个坑。"老潘说，"一个同事用 AI 生成了用户登录模块，看起来没问题。上线第二天，有人输入了一个超长密码——直接把所有用户的登录状态搞乱了。"

"为什么？"

"AI 生成的代码没做边界检查。它假设用户输入都是合理的，没考虑'如果有人恶意输入 10 万个字符会怎样'。这种坑，AI 不会告诉你，因为它自己也不知道。"

老潘在白板上写了几个点：

"AI 不能帮你做这些决策：
- 你的团队有几个人？AI 可能生成一个需要 10 个人维护的架构
- 你的业务会怎么变化？AI 只会生成'现在能用'的代码，不会考虑扩展性
- 出了问题谁负责？AI 不会帮你背锅
...
从第一行 `HelloWorld` 到现在，我们走过了这样一条路：
...
| AI 协作 | 直接复制 AI 生成的代码 | 先理解，再审查，再使用 |

这不是要你"必须写出完美代码"。

而是要你**开始有意识地思考**：我写的代码，三个月后还能看懂吗？别人能接手吗？改一个需求要动多少地方？
...
从机器语言的 0 和 1，到高级语言的 `printf`，再到面向对象的 `class`，最后到今天的 AI 辅助——软件工程的演进始终在做一件事：**提升抽象层次，控制复杂度**。

但抽象层次的提升从来不是免费的。Java 的类型系统看起来是"麻烦"，实则是你和编译器之间的契约——它把错误从运行时提前到编译期，让你在代码上线前就发现问题。这种"严格"在小型脚本里可能显得多余，但在团队协作和长期维护中价值巨大。

更重要的是，你开始理解"能跑"和"可维护"之间的鸿沟。好的命名、解释"为什么"的注释、清晰的代码结构——这些不是"锦上添花"，而是工程的基本功。在 AI 能秒写代码的今天，这些基本功反而更加重要：AI 生成代码，但你必须负责审查、测试、维护。

最后，你学会了锚点（anchor）——把工程决策的"主张"与"可验证的证据"绑定在一起。这是贯穿整本书的思维工具。

下周我们将进入**类设计与领域建模**。你会用类来抽象现实世界的概念（Task、User、Project），写出第一份 ADR（架构决策记录），并发现：Java 的类不只是"代码的组织方式"，它是你理解问题域的工具。
