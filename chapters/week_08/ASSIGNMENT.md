...
> 如果你使用 `starter_code`：起步代码已经预先抽出了 `OrderRepository` 接口和内存实现，目的是让你专注练习价格、运费、折扣和协调逻辑的重构。不要把数据库/JDBC 代码重新塞回 `OrderProcessor`；请在现有接口边界内继续拆分职责。
...
1. **提取方法（Extract Method）**
   ...
   - 每个方法只做一件事
   - 方法命名要清晰表达意图
   - 保持 starter 现有公开行为：`processOrder(...)` 返回 `Order`

2. **移动方法（Move Method）**
   - 将订单保存和查询保持在 `OrderRepository` 抽象之后
   - 将价格、运费、折扣计算逻辑移到合适的位置
...
```java
// OrderRepository.java - 负责订单保存和查询（starter 已提供接口）
public interface OrderRepository {
    void save(Order order);
    List<Order> findByCustomerId(String customerId);
}

// PriceCalculator.java - 负责价格计算（可由你新增）
public class PriceCalculator {
    public double calculateItemPrice(String item) { ... }
}

// ShippingCalculator.java - 负责运费计算（可由你新增）
...
public class OrderProcessor {
    ...
    // 为了兼容 starter 自带测试，保留这个构造器；如需新增依赖，可增加重载构造器。
    ...
    public Order processOrder(String customerId, List<String> items,
        ...
        // 现在这个方法应该很简洁，只负责协调各个组件，并返回保存后的 Order
    }
}
```
...
- 如果你遇到困难，可以参考 `starter_code/src/main/java/com/campusflow/OrderProcessor.java` 和 `starter_code/src/test/java/com/campusflow/OrderProcessorSmokeTest.java` 中的提示
