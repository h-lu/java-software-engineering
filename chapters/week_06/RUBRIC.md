...
| **AI 协作能力** | 20%（可选附加分） | AI 审查深入，发现关键问题（如边界遗漏、弱断言）；能写出让 AI 测试通过的"错误实现"；反思深刻 | 完成 AI 审查流程，发现 3 个以上问题；修复后的代码质量良好 | 简单使用 AI，审查不够深入；仅发现表面问题 | 未使用 AI 或误用（直接提交 AI 输出无审查） |

**注**：AI 协作练习为可选附加分项（+10 分）。基础评分基于：功能完成度（40%）、测试质量（40%）、代码规范（20%）。
