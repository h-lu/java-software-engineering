...
> ## 2. 让程序记住东西
>
> 上一节我们让 Java 说了"Hello, World!"——但你发现没有？那个程序是"一次性"的。它说完就忘了，下一行代码根本不知道上一行发生了什么。
>
> 现在试一个稍微有意思的事情：你想让程序先问用户叫什么名字，然后跟他打招呼。问题来了——`Scanner` 读到名字之后，怎么"记住"它，留到 `println()` 的时候再用？
>
> 答案是**变量**（variable）：
>
> ```java
...
> ```
>
> `name` 就是一个变量。你可以把它想成一个写着名字的便利贴：`nextLine()` 把用户敲进去的文字写在这张便利贴上，后面的代码随时可以去看。
>
...
> ```
>
> 报错：`java: cannot find symbol: variable score`
>
> 原因：使用变量前必须先声明。Java 编译器找不到名为 `score` 的变量。
>
> **大小写不匹配**：
>
> ```java
...
> ```
>
> 报错：`java: cannot find symbol: variable Name`
>
...
> ```java
> String name = "小明";
> System.out.println(Name);   // 注意这里的大写 N
> ```
>
> 编译后 Java 会报错：
>
> ```
> java: cannot find symbol: variable Name
> ```
>
> 你定义的是 `name`（小写），但用的是 `Name`（大写）。在 Java 的世界里，这是两个完全不同的名字——就像"张伟"和"张威"在户口本上是两个人。Java 不会猜你"大概是想说那个 name 吧"，它会直接告诉你"我不认识 Name"。
>
> 所以养成一个习惯：**变量名全部用小写字母加驼峰**（比如 `userName`），这样永远不会在大小写上翻车。

**为什么好？**
- 错误是在"学生正在写代码"的语境中出现的，不是突然跳出来的清单。
- 报错信息之后有"拟人化"的解释（"Java 不会猜"），帮助读者建立直觉。
...
> ### 3.4 小结
>
> - `Scanner.nextLine()` 获取用户输入，返回字符串
> - 数字运算前需要用 `Integer.parseInt()` 转换成整数
> - 如果输入的内容无法转换，会报 `NumberFormatException`

### ✅ 好（回顾 + 展望）

> 这一节你学会了让程序"等人说话"。`Scanner` 拿到的东西永远是文字（字符串），如果你要拿它做数学运算，得先用 `Integer.parseInt()` 告诉 Java"这其实是个数字"。
>
> 到这里，你已经掌握了输出（`System.out.println`）、记忆（变量）和输入（`Scanner`）。下一节我们来聊聊文字本身——Java 里的字符串比你想的能干得多。
...
- 第 1 节（println）：先硬编码输出一张固定名片
- 第 2 节（变量）：把姓名、年龄存进变量，名片变得"可配置"
- 第 3 节（Scanner）：让用户自己输入信息，名片变成"交互式"
- 第 4 节（字符串）：用字符串拼接和格式化美化名片
- 第 5 节（错误）：故意触发各种编译错误，学会读报错信息
...
> 小北照着教材敲完代码，自信满满地编译运行——然后 Java 编译器用红色大字教育了他：
>
> ```
> java: cannot find symbol: variable Name
> ```
...
>
> 你发现问题了吗？`name` 和 `Name` 在 Java 看来是两个完全不同的名字。
>
> 阿码在旁边探头看了一眼："那如果我故意用大写字母当变量名呢？比如 `String Name = "阿码"`——这样合法吗？"
>
> 好问题——合法，但不推荐。Java 社区有个约定：普通变量全部用小写加驼峰（camelCase）。
...
>
> 还是 `for`，但这次遍历的不是索引，而是一个**列表**（List）：
>
> ```java
> List<String> names = List.of("小北", "阿码", "老潘", "小红", "小明");
> for (String name : names) {
>     System.out.println("到！" + name);
> }
> ```
>
> 同样的 `for`，不同的数据源。这就是集合的威力——它给了你一个"装东西的容器"。

**为什么好？**
- 从上周的成果（`for` + 索引循环）自然过渡到本周的新概念（列表）。
- 读者感受到"原来 `for` 还能这样用"——旧知识在新场景下焕发新意。
- 用循环角色的名字做列表数据，增加代入感。

---

## 范例 8：CampusFlow 超级线进度写法

### ❌ 坏（与正文脱节）

> ## CampusFlow 进度
>
> 本周给 CampusFlow 加了异常处理。代码如下：
>
> ```java
> try {
...
> ## CampusFlow 进度
>
> 到目前为止，CampusFlow 有一个致命弱点：如果你在菜单选择时输入了"abc"而不是数字——它直接崩溃。
>
> 这正好是本周学的内容派上用场的地方。给 CampusFlow 穿上"防弹衣"：
>
> ```java
...
>                 return choice;
>             }
>             System.out.println("请输入 1 到 4 之间的数字。");
>         } catch (NumberFormatException e) {
>             System.out.println("这不是数字，再试一次。");
>         }
>     }
> }
> ```
>
> 现在无论用户输入什么乱七八糟的东西，CampusFlow 都不会崩了——它会耐心地等你输入正确的选项。
>
> 老潘看到这段代码会说什么？"这就对了。用户永远会做你想不到的事情。"

**为什么好？**
- 从 CampusFlow 的真实"痛点"出发，让本周知识（异常处理）有了用武之地。
- 改进有前后对比（之前会崩，现在不会）。
- 用老潘的点评收束，增加工程视角。
